...
const (
	...
	// ReasonCodeV3GrantedQoS0 indicates that the subscription was accepted and the maximum QoS sent
	// will be QoS 0.
	ReasonCodeV3GrantedQoS0 ReasonCode = 0x00

	// ReasonCodeV3GrantedQoS1 indicates that the subscription was accepted and the maximum QoS sent
	// will be QoS 1.
	...
	// ReasonCodeV3IdentifierRejected indicates that the client identifier is correct UTF-8 but not
	// allowed.
	...
	// ReasonCodeV3BadUsernamePassword indicates that the data in the UserName or Password is
	// malformed.
...
const (
	...
	// ReasonCodeV5GrantedQoS1 indicates that the subscription was accepted and the maximum QoS sent
	// will be QoS 1.
	...
	// ReasonCodeV5NoMatchingSubscribers indicates that the message was accepted but there are no
	// subscribers.
	...
	// ReasonCodeV5MalformedPacket indicates that data within the CONNECT packet could not be
	// correctly parsed.
	ReasonCodeV5MalformedPacket ReasonCode = 0x81

	// ReasonCodeV5ProtocolError indicates that data in the packet does not conform with the V5.0
	// specification.
	...
	// ReasonCodeV5KeepAliveTimeout indicates that connection is closed because no packet has been
	...
	// ReasonCodeV5TopicNameInvalid indicates that the topic name is correctly formed, but is not
	// accepted.
	...
	// ReasonCodeV5ReceiveMaximumExceeded indicates that has been received more than Receive Maximum
	// publication for which it has not sent PUBACK or PUBCOMP.
	ReasonCodeV5ReceiveMaximumExceeded ReasonCode = 0x93

	// ReasonCodeV5TopicAliasInvalid indicates that has received a PUBLISH Packet containing a topic
	...
	// specified in a Maximum QoS in the CONNACK.

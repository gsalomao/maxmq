...
const (
	...
	// ReasonCodeV3GrantedQoS0 indicates that the subscription was accepted and the maximum QoS sent will be QoS 0.
	ReasonCodeV3GrantedQoS0 ReasonCode = 0x00

	// ReasonCodeV3GrantedQoS1 indicates that the subscription was accepted and the maximum QoS sent will be QoS 1.
	ReasonCodeV3GrantedQoS1 ReasonCode = 0x01

	// ReasonCodeV3GrantedQoS2 indicates that the subscription was accepted and any received QoS will be sent.
	...
	// ReasonCodeV3IdentifierRejected indicates that the client identifier is correct UTF-8 but not allowed.
	ReasonCodeV3IdentifierRejected ReasonCode = 0x02

	// ReasonCodeV3ServerUnavailable indicates that the MQTT service is unavailable.
	ReasonCodeV3ServerUnavailable ReasonCode = 0x03

	// ReasonCodeV3BadUsernamePassword indicates that the data in the UserName or Password is malformed.
...
const (
	...
	// ReasonCodeV5NormalDisconnection indicates to close the connection normally and do not send the Will Message.
	ReasonCodeV5NormalDisconnection ReasonCode = 0x00

	// ReasonCodeV5GrantedQoS0 indicates that the subscription was accepted and the maximum QoS sent will be QoS 0.
	ReasonCodeV5GrantedQoS0 ReasonCode = 0x00

	// ReasonCodeV5GrantedQoS1 indicates that the subscription was accepted and the maximum QoS sent will be QoS 1.
	ReasonCodeV5GrantedQoS1 ReasonCode = 0x01

	// ReasonCodeV5GrantedQoS2 indicates that the subscription was accepted and any received QoS will be sent.
	ReasonCodeV5GrantedQoS2 ReasonCode = 0x02

	// ReasonCodeV5DisconnectWithWillMessage indicates to close the connection, but the server shall send the Will
	// Message.
	ReasonCodeV5DisconnectWithWillMessage ReasonCode = 0x04

	// ReasonCodeV5NoMatchingSubscribers indicates that the message was accepted but there are no subscribers.
	...
	// ReasonCodeV5UnspecifiedError indicates that the server does not wish to reveal the reason for the failure, or
	// none of the other codes apply.
	ReasonCodeV5UnspecifiedError ReasonCode = 0x80

	// ReasonCodeV5MalformedPacket indicates that data within the CONNECT packet could not be correctly parsed.
	ReasonCodeV5MalformedPacket ReasonCode = 0x81

	// ReasonCodeV5ProtocolError indicates that data in the packet does not conform with the V5.0 specification.
	ReasonCodeV5ProtocolError ReasonCode = 0x82

	// ReasonCodeV5ImplementationError indicates that the packet is valid but is not accepted by the server.
	ReasonCodeV5ImplementationError ReasonCode = 0x83

	// ReasonCodeV5UnsupportedProtocolVersion indicates that the server does not support the version of the MQTT
	// protocol requested by the client.
	ReasonCodeV5UnsupportedProtocolVersion ReasonCode = 0x84

	// ReasonCodeV5InvalidClientID indicates that the Client ID is a valid string but is not allowed by the server.
	ReasonCodeV5InvalidClientID ReasonCode = 0x85

	// ReasonCodeV5BadUserNameOrPassword indicates that the server does not accept the UserName or Password specified
	// by the client.
	...
	// ReasonCodeV5BadAuthMethod indicates that the authentication method is not supported or does not match the
	// authentication method currently in use.
	ReasonCodeV5BadAuthMethod ReasonCode = 0x8C

	// ReasonCodeV5KeepAliveTimeout indicates that connection is closed because no packet has been received for 1.5
	// times the Keepalive time.
	ReasonCodeV5KeepAliveTimeout ReasonCode = 0x8D

	// ReasonCodeV5SessionTakeOver indicates that another connection using the same client identified has connected.
	ReasonCodeV5SessionTakeOver ReasonCode = 0x8E

	// ReasonCodeV5TopicFilterInvalid indicates that the topic filter is correctly formed but is not allowed.
	ReasonCodeV5TopicFilterInvalid ReasonCode = 0x8F

	// ReasonCodeV5TopicNameInvalid indicates that the topic name is correctly formed, but is not accepted.
	ReasonCodeV5TopicNameInvalid ReasonCode = 0x90

	// ReasonCodeV5PacketIDInUse indicates that the specified packet ID is already in use.
	...
	// ReasonCodeV5ReceiveMaximumExceeded indicates that has been received more than Receive Maximum publication for
	// which it has not sent PUBACK or PUBCOMP.
	ReasonCodeV5ReceiveMaximumExceeded ReasonCode = 0x93

	// ReasonCodeV5TopicAliasInvalid indicates that has received a PUBLISH Packet containing a topic alias which is
	...
	// ReasonCodeV5QuotaExceeded indicates that an implementation or administrative imposed limit has been exceeded.
	ReasonCodeV5QuotaExceeded ReasonCode = 0x97

	// ReasonCodeV5AdministrativeAction indicates that the connection is closed by administrative action.
	ReasonCodeV5AdministrativeAction ReasonCode = 0x98

	// ReasonCodeV5PayloadFormatInvalid indicates that the payload format does not match the specified payload format
	// indicator.
	...
	// ReasonCodeV5QoSNotSupported indicates that the client specified a QoS greater than the QoS specified in a Maximum
	// QoS in the CONNACK.
	ReasonCodeV5QoSNotSupported ReasonCode = 0x9B

	// ReasonCodeV5UseAnotherServer indicates that the client should temporarily use another server.
	...
	// ReasonCodeV5SharedSubscriptionsNotSupported indicates that the server does not support Shared Subscriptions.
	...
	// ReasonCodeV5MaximumConnectTime indicates that the maximum connection time authorized for this connection has been
	// exceeded.
	ReasonCodeV5MaximumConnectTime ReasonCode = 0xA0

	// ReasonCodeV5SubscriptionIDNotSupported indicates that the server does not support Subscription Identifiers.
	ReasonCodeV5SubscriptionIDNotSupported ReasonCode = 0xA1

	// ReasonCodeV5WildcardSubscriptionsNotSupported indicates that the server does not support Wildcard Subscriptions.

...
// New returns an instance of the Snowflake.
func New(machineID int) *Snowflake {
	if machineID < 0 || machineID > machineIDMask {
		panic(fmt.Sprintf("invalid machineID (must be 0 ≤ id ≤ %d)", machineIDMask))
	}

	return &Snowflake{machine: uint64(machineID << machineShift)}
}
...
// NextID returns a new ID following the Snowflake specification.
